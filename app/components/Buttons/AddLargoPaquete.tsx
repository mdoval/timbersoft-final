...
import { addLargoPaquete } from "@/utils/largosPaqueteFunctions";
import { ILargoPaquete } from "@/types/tipos";

export function AddLargoPaquete() {
  const router = useRouter();
  const [modalOpen, setModalOpen] = useState<boolean>(false);
  const [newLargo, setNewLargo] = useState<ILargoPaquete>({ tamanio: 0 });
  ...
  const handleSubmitNewCategoria: FormEventHandler<HTMLFormElement> = async (
    ...
    e.preventDefault();
    try {
      await addLargoPaquete(newLargo);
        ...
        <form onSubmit={handleSubmitNewCategoria}>
          <div className="modal-action">
            <div className="flex flex-col space-y-5 w-full">
              <h3 className="font-bold text-lg">Agregar Largo Paquete</h3>
